...
func TestAddUserFavouriteHandler_AddUserFavouriteAssetController(t *testing.T) {
	...
	tests := []struct {
		...
		args                     args
		requestBody              []byte
		mockRequestData          *AddUserFavouriteAssetRequest
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			handler := &AddUserFavouriteHandler{
				UserFavouriteService: mockService,
				AssetTypeValidator:   assetTypeValidator,
				logger:               logger,
			}
			sut := handler.AddUserFavouriteAssetController
...
func TestAddUserFavouriteHandler_AddUserFavouriteAssetControllerHasBadRequestError(t *testing.T) {
	logger := logger.NewLogger(context.Background())
	mockCtrl := gomock.NewController(t)
	defer mockCtrl.Finish()
...
func TestAddUserFavouriteHandler_AddUserFavouriteAssetControllerHasServiceError(t *testing.T) {
	logger := logger.NewLogger(context.Background())
	mockCtrl := gomock.NewController(t)
	defer mockCtrl.Finish()
	...
	"assetType":"chart"
			...
			expected: json.RawMessage(`{"errorMessage":"error in adding user favourite assets"}
`),

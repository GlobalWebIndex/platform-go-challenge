...
type EmailService interface {
	...
	CheckCode(email string, code int64) error
}
...
func NewEmailVerificationService(dao repository.DBHandler) EmailService {
	return &emailService{dao: dao}
}

func (e *emailService) VerifyEmail(email string) error {
	// email, err := e.dao.NewUserQuery().GetEmailByUserID(userID)
	// if err != nil {
	// 	return err
	// }

	_, err := e.dao.NewBusinessQuery().GetBusiness(email)
	if err != nil {
		return err
	}
...
func (e *emailService) CheckCode(email string, code int64) error {
	_, err := e.dao.NewBusinessQuery().GetBusiness(email)
	if err != nil {
		return err
	}
